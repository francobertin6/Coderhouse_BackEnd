...
class CartManager {
    constructor(productsIds, carritoJson) {
    ...
    AgregateProduct_to_cart(cartId, productId) {
        ...
        if (findCart !== undefined && findProduct !== undefined) {
            let quantityCheck = findCart.products.find((element) => {
                return element.product === productId;
            });
            console.log(quantityCheck);
            if (quantityCheck !== undefined) {
                let findIndex = findCart.products.findIndex((element) => element === quantityCheck);
...
const productsId = (products) => {
    if (products === undefined) {
        ...
        return products.map((element) => { return element.id; });
    }
};
